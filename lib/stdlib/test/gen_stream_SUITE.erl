...
%%%--------------------------------------------------------
%%% Here is the tested gen_stream behaviour.
%%%--------------------------------------------------------
%% init([]) ->
%%     {ok, []};
%% init(ignore) ->
...
%%     ?line {ok,Pid0} = gen_stream:start_link(?MODULE, [], []),
%%     ?line {'EXIT',{{shutdown,reason},_}} =
%%  	(catch gen_stream:call(Pid0, shutdown_reason)),
%%     receive {'EXIT',Pid0,{shutdown,reason}} -> ok end,

%%     %% This crash should not generate a crash report.
%%     ?line {ok,Pid1} = gen_stream:start_link(?MODULE, {state,state1}, []),
%%     ?line {'EXIT',{{shutdown,stop_reason},_}} =
%% 	(catch gen_stream:call(Pid1, stop_shutdown_reason)),
%%     receive {'EXIT',Pid1,{shutdown,stop_reason}} -> ok end,

%%     %% This crash should not generate a crash report.
%%     ?line {ok,Pid2} = gen_stream:start_link(?MODULE, [], []),
%%     ?line {'EXIT',{shutdown,_}} =
%%  	(catch gen_stream:call(Pid2, exit_shutdown)),
%%     receive {'EXIT',Pid2,shutdown} -> ok end,

%%     %% This crash should not generate a crash report.
%%     ?line {ok,Pid3} = gen_stream:start_link(?MODULE, {state,state3}, []),
%%     ?line {'EXIT',{shutdown,_}} =
%% 	(catch gen_stream:call(Pid3, stop_shutdown)),
...
%%     %% from gen_server.
%%     ?line {ok,Pid4} = gen_stream:start(?MODULE, {state,state4}, []),
